...
namespace adk {
	 ...
	 *
	 * @param item Identifier of the item, include namespace
	 *
	 * @param icon Name of the texture defined in `item_texture.json`
	 */
	void SimpleItem(std::string item, const std::string icon) {
		auto registry = GetIDs();

		if (!(std::find(registry.begin(), registry.end(), item) != registry.end())) {
			log::error("No item found: {} - Please check if this block exists", item);
			exit(EXIT_FAILURE);
		}
