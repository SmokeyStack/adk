...
namespace adk {
	...
	void ComponentBoxCollision::EnsureValidity(Vector3 origin, Vector3 size) {
		if (origin.GetX() < -8 || origin.GetX() > 8 ||
			...
			log::error("\t{} - 'origin' must be in the range (-8, 0, -8) to (8, 16, 8), inclusive. Current value provided: {}", "minecraft:collision_box", origin.Get());
			exit(EXIT_FAILURE);
		}
		...
		if ((origin.GetX() + size.GetX() < -8) ||
			...
			(origin.GetZ() + size.GetZ() > 8)) {
			log::error("\t{} - 'origin' + 'size' must be in the range (-8, 0, -8) to (8, 16, 8), inclusive. Current value provided: {}", "minecraft:collision_box", CombineOriginAndSize(origin, size));
			exit(EXIT_FAILURE);
		}
	}
